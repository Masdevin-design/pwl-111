    ...
    public function costs()
    {
        ...
        try {
            ...
            return $this->response->setStatusCode(502)->setJSON([
                'message' => 'Gagal menghubungi RajaOngkir: ' . $th->getMessage()
            ]);
        }
        ...
        foreach ($data as $item) {
            if (isset($item['costs']) && is_array($item['costs'])) {
                foreach ($item['costs'] as $costItem) {
                    ...
                    $results[] = [
                        ...
                        'description' => $costItem['description'] ?? ($item['name'] ?? $courier),
                        'cost'        => $costDetail['value'] ?? 0,
                        'etd'         => $costDetail['etd'] ?? '-'
                    ];
                }
            ...
            $results[] = [
                ...
                'description' => $item['description'] ?? ($item['name'] ?? $courier),
                'cost'        => $item['cost'] ?? ($item['price'] ?? 0),
                'etd'         => $item['etd'] ?? '-'
            ];
        }

        if (empty($results) && isset($response['message'])) {
            return $this->response->setStatusCode(502)->setJSON([
                'message' => $response['message']
            ]);
        }

        if (empty($results) && isset($response['meta']['message'])) {
            return $this->response->setStatusCode(502)->setJSON([
